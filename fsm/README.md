...
| `transitions` | 允许的转换（`{ from: [to...] }`）。**省略 = 任意转换都允许；配了但缺项 = 一个都不许转** |
...
> ⚠️ **配了 `transitions` 但当前状态在表里没有条目时，`can()` 一律返回 `false`。**
> 早期版本把"缺项"和"压根没配表"当成一回事，都按"允许任意"处理，
> 于是新增状态忘了补一行时，该状态**能转到任意状态**——白名单形同虚设，
> 而 `can()` 返回 true 让宿主以为校验通过了。
> 现在缺项 = 一个都不许转，漏配会在运行时立刻暴露。
> `findUnreachable()` 与 `can()` 共用同一套判定（内部 `_targetsOf`），
> 不会出现"报告说可达、运行时却拒绝"的自相矛盾。
