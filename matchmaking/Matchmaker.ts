...
export class Matchmaker {
   ...
   *
   * 【为什么不能只调 `clear()`】
   * `clear()` 只清空队列（这是它的契约，不该改），
   * 但撮合器还持有**配置闭包**——`_balance` / `_teamsPerMatch` 之外，
   * 还有注入的随机源和"撮合成功"的回调。
   * 那些东西的生命周期通常比撮合器长（随机源尤其是全局单例），
   * 但反过来也成立：如果撮合器被一个长期存活的对象持有，
   * 它就会一直把整个配置闭包拖在内存里。
   *
   * 【为什么顺手把队列清掉】
   * 卸载后仍可查询是危险的：一个"已卸载"的撮合器继续返回上一轮的队列，
   * 会让玩家被塞进一个根本不会开的对局里。
   * 这里让 destroy 之后的行为与"刚 new 出来"一致。
   */
  destroy(): void {
    this.clear();
  }
  ...
  private _balanceTeams(teams: Queued[][]): void {
    ...
    while (improved && guard < 200) {
      ...
      /**
       * 【⚠️ 这里原本有一句空 if（注释写着"保证 hi 更强"，但块内一行代码都没有）】
       *
       * 它既不交换也不 return，注释承诺的事一件没做。
       * 紧随其后的 `ratings[hi] >= ratings[lo] ? hi : lo` 已经完整处理了大小关系，
       * 所以删掉它不改变任何行为（已用测试固化：强/弱判定仍然正确）。
       *
       * 留着它的唯一后果是误导：读代码的人会以为 `hi` 在这里被"摆正"了，
       * 于是在下面写逻辑时**假设 hi 一定更强**——而实际情况由下一行决定。
       * 空块 + 与事实不符的注释，比没有注释更糟。
       */
      const hi = worstI;
      const lo = worstJ;
