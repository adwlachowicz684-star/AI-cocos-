...
| 流水无上限 | 长会话累积几十万条，内存与 `netSpent()` 同步劣化 | 已处理：`logLimit`（默认 200，超出丢最老的） |

### 构造参数

```typescript
new Shop({ logLimit: 200 });   // 流水上限，clampNum 收口，默认 200
```

`logLimit` 与 `currency` 单元的 `logLimit` 取同一个默认值，口径一致。
传 `NaN` 会回落到 200（不会退化成"无上限"）。

> ⚠️ 超限时丢的是**最老的**那几条。`netSpent()` 依赖流水做对账，
> 所以它不是"精确的历史账本"，而是"最近的 N 条"。
> 需要对全量账请自己在外部落库。
