...
**① 位宽上限 32，且 31 / 32 位都真的能用。**
`writeBits` 用 JS 的 32 位位运算，超过会**静默溢出**（`(1<<33) === 1<<1`）。
这种错误极难发现——值写进去了，读出来是错的。所以 schema 构造时就校验。

> ⚠️ **31 / 32 位曾经是不可用的**：范围用的是 `1 << bits`，
> 而 `1 << 31` 是**负数**、`1 << 32` 绕回 `1 << 0`。
> 于是 `uint(31)` 的上界被算成 -2147483649、`uint(32)` 算成 0，
> 写任何合法值都抛"越界"。现在统一用 `2 ** bits` 的乘方语义。
> **注意 `1 << bits` 这个坑在全库任何地方都成立，不只是这里。**
...
`uint` / `int` / `float` 一视同仁：越界即抛错。
`float` 是唯一可以例外的——传 `float(min, max, step, def, { clamp: true })`
显式声明"我就是要截断"，让这件事写进代码里而不是让人去猜。

**④ 量化档位 = `floor((max-min)/step) + 1`，读写两端按同一个上界收口。**
`(max-min)/step` 不是整数时（如 `float(0, 1.8, 0.5)`），
`max` 本身不在量化网格上，写入会落到最近的档位（1.8 → 1.5）。
这是**量化误差**不是截断：它小于一个 `step`，且读写对称——
`read()` 能读到的最大值恒 ≤ `max`（早期版本能读出 2.0 > max）。
...
| enum 索引越界 | **曾经**返回 undefined，后续崩溃 | 退化为构造时传入的 `def` 默认值 |
