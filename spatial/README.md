...
| 元素 < 50 也用 | 反而更慢 | 直接遍历 |
| 以为传了 `capacity` 就有容量上限 | 内存无上限增长 | **该字段当前未实现**，见下 |

> ⚠️ **`SpatialHashOptions.capacity` 声明了但从未生效。**
>
> 全文检索确认：`capacity` 只在接口定义处出现一次，
> 构造函数和所有方法都没有读取它。
> 传 `new SpatialHash({ cellSize: 8, capacity: 2 })` 之后，
> `update()` 依然会无上限地往里塞条目。
>
> 这比"没有这个配置"更糟——调用方会以为已经做了容量保护，
> 于是拿它做内存预算。**不要依赖它。**
>
> 去留待裁决：实现成硬上限会改变 `update()` 的行为，
> 从接口删除则让已传该字段的调用方编译报错。两者都是 breaking。
> 在此之前，需要容量控制请在调用方自己限制登记数量。
