...
export class DashController {
  ...
  constructor(opts: DashOptions = {}) {
     ...
     * - `falloff = -1` → p = 0 → `s(u) = 1 - (1-u)^0 ≡ 0`（中前段位移恒 0，全靠收尾分支补齐）
     *
     * 实测 falloff = -1 的表现不是"冲不动"，而是**瞬移**：
     * `s(u) ≡ 0` 让分段位移在中前段几乎全为 0，
     * 位移全部堆到末段（`endBrake > 0` 时集中在最后 15%，`endBrake = 0` 时压在收尾那一帧），
     * 于是衰减曲线、无敌帧节奏、残影间隔全部失效——
     * 但**总距离仍精确等于 `distance`**，所以从数值上看不出错，极难排查。
     *
     * ⚠️ 措辞口径：这里不是"位移恒 0"（`_progressAt(1) || 1` 那个兜底在
     * `endBrake > 0` 时压根不会触发，总位移是对的），而是**曲线退化成单帧瞬移**。
     * 按"位移恒 0"去修会改错地方。
