...
| 错误分级 | `CommandError`（用户输入错，打印提示）vs 内部异常（**默认只打印**，可 `rethrow: true` 改为向上抛） |
...
// 其他任何异常 → 默认也只打印一行 `✗ 命令内部错误：<堆栈>`，不向外抛
//                需要接 CrashReporter 时传 `rethrow: true`
```

混在一起的话，真 bug 会被当成"玩家输错了"而静默吞掉。

> ⚠️ **为什么内部异常默认不抛（行为变更）**
> 控制台是"运行时调试的最后一道防线"，本该吞掉一切、转成一行红字。
> 老实现在打印完错误信息后又把异常抛回调用方，
> 于是它落在 UI 的输入事件处理器里——**一行打错的命令就能让整个输入系统崩掉**，
> 而错误已经被打印过一次（重复暴露）。
> 需要崩溃上报链路捕获时显式传 `rethrow: true`。
...
| `rethrow` | false | 命令内部异常是否继续向上抛（默认只打印） |
