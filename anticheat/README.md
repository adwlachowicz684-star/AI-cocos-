...
| `reset()` | 重置 |
| `destroy()` | 释放（无外部资源，等价于 `reset()`；为统一收尾入口而提供） |

> ⚠️ **非法样本被"跳过不计"，既不加连击也不清零。**
> `dt` 非法（NaN / 0 / 负数）和**坐标 NaN** 都属于非法样本。
>
> 早期 dt 守卫写成否定式 `dtSec <= 0`，**拦不住 NaN**（`NaN <= 0` 恒 false）：
> 算出 `speed = NaN` → `exceeded = NaN > allowed` 恒 false → 走 else → `_strikes` 被清零。
> 作弊者只要在任意一次上报里塞一个 NaN 时间戳，就能把连击洗掉、永远够不到 `strikeThreshold`。
> 坐标 NaN 是同一条链上的第二个入口（dt 守卫挡不住它）。
>
> 【为什么是"跳过"而不是"清零"】非法样本既不是合规证明也不是违规证据，
> 它没有信息量；把它算成"没超速"等于白送一次清零。
