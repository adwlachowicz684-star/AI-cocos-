...
`addImpulse` 的第三个参数是强度上限，建议设为 `maxSpeed × 4` 左右。
...
**⚠️ 它限制的是"朝向"追上移动方向的快慢，不是"移动方向"本身。**
角色照样立刻朝新方向加速位移，只是头（或模型）转过去需要时间。
这正是"Soul-like / 坦克式操作"想要的：脚下快、转身有重量感。

| 取值 | 效果 |
|---|---|
| `0` / 不配（**默认**） | 瞬时 —— 保持既有行为，老代码一行不改 |
| `Math.PI` ≈ 3.14 | 半圈/秒，掉头约 1 秒，明显有转身过程 |
| `Math.PI * 4` ≈ 12.6 | 两圈/秒，几乎瞬时但能抹掉抖动 |
| `Math.PI * 0.5` ≈ 1.57 | 掉头约 2 秒，很重，适合大型敌人 / 载具 |

```typescript
const m = new CharacterMover({ turnRate: Math.PI });
m.update(dt, dirX, dirY);
sprite.angle = -m.facing * 180 / Math.PI;   // Cocos 的 angle 顺时针为正
```

**若要"移动方向本身平滑改变"，该调的是 `accel` 与 `turnBoost`，不是这里。**

配套新增了 `facing` 的 setter，可一次设对朝向（**不受限速**）：

```typescript
m.facing = Math.PI;   // 出生/切场景/进入对话时锁朝向
```

> 开局 `_facing` 是 0（朝右）。若角色第一次就往左走，
> 配了 `turnRate` 会从 0 慢慢转过去——玩家会看到"出生时莫名转身"。
> 用 setter 可以避免。

**转向走最短路径**：从 170° 转到 -170° 只转 20°，不会绕 340° 的一大圈。
坏 `dt`（NaN / Infinity / 负数）会让这一帧不转，但**不会**把朝向毒化成 NaN。
...
| `addImpulse(ix, iy, max?)` | 施加外力（击退、爆炸、传送带） |
...
| `facing` | 朝向（弧度；静止时保持上一次）。**可读可写**，setter 立即生效且不受 `turnRate` 限速 |
