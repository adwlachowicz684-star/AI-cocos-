...
> 本文件是**第二次精审 273 条**中分配给窗口 W4 的部分。
...
```
achievement  audio  command  condition  curve  expression  hitbox  minimap  save  skill-caster  skill-player  subtitle
```
...
新建 `tests/run_phase10_w4.ts`，并**在文件内导出** `runPhase10W4Tests()`：

```ts
export function runPhase10W4Tests(): void {
  // ...
}
```
...
| **文件命名** | `run_phase10_w4.ts` / `result_W4.md`，带你的窗口号，避免撞名 |
