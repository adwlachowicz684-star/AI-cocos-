...
## 6. 回应主审核人终审裁决（`CHIEF_FINAL_VERDICT.md`，2026-09-08 04:12）

### 6.1 与本窗口直接相关的三条裁决

| # | 裁决 | 我的确认 |
|---|---|---|
| **A3** | 重写 `verify_W8-B.md` 为对 W8-A 的逐条验收 | ✅ **已闭环**。第 2 版（`1d28f325`，03:53）已完成重写，结论「通过」；本轮又出第 3 版收口（补回退实验原始输出 + 两处自我更正 + C9/C10 回应） |
| **C6** | `binary` float 越界改抛错 —— 采纳改法，闭环 | ✅ 确认。当前 main 断言已同步、全量 4,653/0 |
| **C8** | `meta.requires` 采纳 B 折中（构造期 warn + `unknownRequires`，运行期跳过） | ✅ 采纳的是我提的折中方案，W8-A 已实现；我实测合法配置 0 告警 / 笔误 1 告警 |

### 6.2 ⚠️ 两条提请回看（详见 `verify_W8-B.md` §7）

**C9（`autoquality` AQ6 不合并）**——裁决依据是我第 1 版里**已被我自己撤回**的意见。三条事实：

1. `audit/handoff_W8-A.md:194` 是**明文指令**："应合并（AutoQuality 内部持有 FpsMeter 即可）"，
   不是 W8-A 自选的重构。我第 1 版写"未实测（属重构项）"时没回去读任务书，这是我的疏漏。
2. 合并已落地且全绿：全量 **4,653 / 0**。
3. C9 要求的安全措施 W8-A 已做：`run_phase10_w8a.ts:587` 专门锁住了
   "AutoQuality 返回 60 / FpsMeter 返回 0"，`_fpsOrIdle` 注释写明了 60 的用意。
   **裁决担心的"合并把 60 抹成 0"没有发生。**

建议改裁为「维持已合并现状 + 保留 60/0 差异断言」。**我不自行改代码**——`autoquality` 非本窗口单元。

**C10（`Chest.destroy()` 不释放 `owned`）**——判"不成立"采纳的是我的原判断，无异议。
但当前实现带来一处**此前没人记录的静默行为**：

```
destroy 前 addOwned('b') → owned = ["a","b"]
destroy 后 addOwned('c') → owned = ["a","b"]   ← 静默吞掉，无报错
```

根因是 `addOwned()` 的守卫 `if (!this._options.owned) return;` 把"已销毁"
和"从未传 owned"当成一回事。建议随 A9（W8-A 补 README）一并处理。

### 6.3 一条我做了但**没有推送**的改动（说明理由）

我在本地把 `tests/run_batch10.ts` 里那条 float 用例**拆成了两条**
（`⚠️ float 越界抛错，绝不静默截断 / 溢出回绕` + `⚠️ float 显式 clamp 时才截断，且不回绕`），
本地实测 3,698 / 0。

**但没有推**，两个理由：

1. **C6 已判闭环**，当前 main 该用例是完整版（`throws` 越界 + `clamp: true` 截断断言都在），
   拆分只是把两半摊开，不是修缺陷。
2. 拆分会让测试总数从终审刚认定的 **4,653 → 4,654**，而根 `README.md` 刚被改成 4,653。
   为一条"更工整"去动已被总盘点的数字，收益不抵混乱成本。

**留作下轮建议**：这条用例把"默认行为"和"显式开关下的行为"挤在一个 `test()` 里，
历史上被并发覆盖时是**两半一起消失**的（本轮已发生过一次）。拆开后即使再被覆盖也只丢一半、且一眼可见。

### 6.4 终审 P-4（快照过期未复检）—— 认领

终审把这条记在 `verify_W8-B` 名下，属实。我第 1 版的核查方法（API 直查交付物是否存在）是对的，
但**结论没有跟着代码状态更新**。改法已写进本报告：验收报告在合并前必须**按 SHA 重新取一次基线**再复检。
本轮第 2/3 版均按 SHA（`fbd27f60`）取基线，不再依赖本地快照。
