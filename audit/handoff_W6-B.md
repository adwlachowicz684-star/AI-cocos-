# 精审返工任务书 · 窗口 W6-B（第 B 组）

> 本文件是**第二次精审 273 条**中分配给窗口 W6-B 的部分。
...
完工后按 `audit/review_B.md` 验收 **W6-A**。
...
```
adapters  dungeon  pathfind  replay
```
...
新建 `tests/run_phase10_w6b.ts`，并**在文件内导出** `runPhase10W6BTests()`：
...
完成后产出 `audit/result_W6-B.md`，每条一行：
...
| **文件命名** | `run_phase10_w6b.ts` / `result_W6-B.md`，带窗口号，避免撞名 |

---

## 7. 完工后的交叉验收

你属于**第 B 组**。修完之后，按 `audit/review_B.md` 验收 **W6-A**（同编号的另一组窗口）。

验收时**不要直接改对方的代码**（会和对方窗口冲突）。发现问题写进 `audit/verify_W6-B.md`，
回报给对方窗口或总审。
