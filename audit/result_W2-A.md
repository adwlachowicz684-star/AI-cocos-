...
**状态：进行中。** 本节第 7.1 之前写的是"W2-B 无交付物"——那是**我基于过期的本地快照
误判的**。复核远端后确认 W2-B 早已交付（`result_W2-B.md` + `run_phase10_w2b.ts`，
独立运行 80 项全绿）。验收结论另见 `audit/verify_W2-A.md`。

> 教训：本地快照落后于远端时，任何"某文件不存在"的判断都不可靠。
> 判断远端状态必须实时查远端，不要基于本地 `git log`。

---

## 8. 推送后复核（在远程最新 main 上重跑）

本窗口推送分两次：

| 提交 | 说明 |
|---|---|
| `ef2d2649` | 11 个单元源码 + `result_W2-A.md` + `run_phase10_w2a.ts` + `_kitmeta.json` |
| `951ac060` | 修正 `_kitmeta.json`（恢复被并发推送覆盖的 `turn` 登记，详见 §9.2）|

推送后**重新下载远程 main 的 tarball** 全量重跑（不是复用本地 `.build/`）：

```
全量回归           → 通过 3696 项，失败 1 项
W2-A 套件独立运行  → 通过 80 项，失败 0 项
11 个单元源码文件  → 与本地版本逐字节一致 ✓
tests/run.ts       → 未被误改 ✓
```

**那 1 项失败与本窗口无关**，鉴定过程见 §9.1。

---

## 9. 两个需要全库注意的发现

### 9.1 远端 main 上的既有红灯：`binary` float（**需 W8-B 处理**）

- **现象**：`run_batch10.ts` › BinarySerializer › 「⚠️ float 会 clamp 而不是溢出回绕」失败，
  报 `[Binary] float 越界：999（范围 -10..10）`。
- **鉴定方法**：下载**未经我改动的纯净远端副本**单独构建跑测，得到**完全相同**的
  `3696 通过 / 1 失败`、同一条失败项 → 排除本窗口责任。
- **根因**：`binary` 是 **W8-B** 的单元。其 `result_W8-B.md` 的 P1-5 写明：
  > `float(0,10,1).write(999)` → 读回 **10**，无任何报错 → 改为默认抛错，
  > 需要截断时显式 `float(...,{clamp:true})`

  该修复**本身正确**（与 README §6③"越界值绝不静默截断"一致，源码注释亦已论证），
  但**漏改了 `tests/run_batch10.ts` 里依赖旧 clamp 行为的那条既有测试**。
- **建议**：由 W8-B 把该旧测试改为断言"默认抛错 + `clamp:true` 时截断"，
  与 P1-5 的新语义对齐。我不改——`batch10` 与 `binary` 都不在我窗口内。

### 9.2 ⚠️ 并发推送会互相覆盖 `_kitmeta.json`（**全库风险，建议总审关注**）

我推送时基于的是旧 main 快照，而期间 W5-A 刚推送过 `_kitmeta.json`。
我的提交整体写入该文件，**静默回退了 W5-A 登记的 `turn → _core`**。

发现后已比对推送前后的两个版本并单独提交修正（`951ac060`），
只恢复 `turn` 一项，其余未登记项留给各自窗口。

**这不是个例**：`result_W2-B.md` 里也写着"重修被并发推送覆盖的 curse 登记"——
说明同类覆盖已发生至少两次。

**建议总审**：`_kitmeta.json` 是 16 个窗口共写的单点文件，谁后推谁覆盖。
两个可选办法：
1. 收归总审统一在合并阶段跑一次 `check-deps.js --fix`（推荐，各窗口不再各自登记）；
2. 或要求各窗口推送前先 rebase 到最新 main 再改这个文件。
