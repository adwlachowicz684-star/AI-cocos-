...
**当前状态**：`main` 编译失败（**189 个 TS 错误**），未修复
...
- [ ] 恢复 28 个纯回退文件（`5efe0031` 版本直接覆盖）
- [ ] 135 个含新内容的文件：逐个人工/窗口三方合并
- [ ] 重做 4 项基础设施（run.ts 注册 / stripCode / _kitmeta 登记 / README 测试数）
- [ ] 修复同步流程：加"推送前先拉 + tsc --noEmit 门禁"
- [ ] 复核第三轮成果（`mover.turnRate`、`run_phase11.ts`）是否幸存
- [ ] 全量回归回到 0 失败后，重算测试总数并更新 README
